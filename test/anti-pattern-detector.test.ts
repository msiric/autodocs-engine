...
function makeConvention(overrides: Partial<Convention>): Convention {
  return {
    ...
    confidence: {
      ...
      total: 86,
      percentage: 100,
      description: "86 of 86 exports (100%)",
    },
    examples: ["useCreateTab", "useUpdateTab"],
    ...overrides,
  };
}

describe("deriveAntiPatterns", () => {
  it("derives 'Do NOT use default exports' from named export convention at 99%", () => {
    const conventions = [
      makeConvention({
        name: "Named exports preferred",
    ...
    expect(antiPatterns[0].rule).toContain("Do NOT use default exports");
    expect(antiPatterns[0].confidence).toBe("high");
    expect(antiPatterns[0].derivedFrom).toBe("Named exports preferred");
  });
...
describe("deriveSharedAntiPatterns", () => {
  it("derives from shared conventions", () => {
    ...
    expect(result[0].rule).toContain("default exports");
  });
});
